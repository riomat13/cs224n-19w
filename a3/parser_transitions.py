...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack = ['ROOT']
        self._buffer = collections.deque(sentence)
        self.dependencies = []

    # this part is added in order to use FIFO data structure(deque)
    # this is specifically for tests
    # which is trying to directly assign a list to the buffer attribute
    @property
    def buffer(self):
        # disallow modify the object directly
        return list(self._buffer)

    @buffer.setter
    def buffer(self, buf):
        self._buffer = collections.deque(buf)
    ...
    def parse_step(self, transition):
        ...
        # 1. Shift
        if transition == 'S':
            self.stack.append(self._buffer.popleft())
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # shallow copy the parser instances
    unfinished_parses = partial_parses[:]
    ...
    while unfinished_parses:
        ...
        for i, transition in enumerate(transitions):
            ...
            if not unfinished_parses[i]._buffer and len(unfinished_parses[i].stack) == 1:
                unfinished_parses[i] = None

        # pop out finished ones
        unfinished_parses = [parser for parser in unfinished_parses if parser is not None]
